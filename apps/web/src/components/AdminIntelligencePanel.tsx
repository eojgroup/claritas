  ...
  return (
    <section className="app-card rounded-xl p-4 sm:p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div><div className="flex items-center gap-2 text-sm font-semibold text-[color:var(--shell-ink)]"><Activity className="h-4 w-4" />Event and Earth Observation operations</div><p className="mt-1 text-xs text-[color:var(--shell-muted)]">Outbox delivery, bounded jobs, provider state, budgets, usage, cache and inspected retries.</p></div>
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-4">
        {[["Outbox", outbox], ["Dead letters", status?.backbone.unresolved_dead_letters ?? 0], ["EO jobs", queued], ["Assets", status?.earth_observation.assets.count ?? 0], ["Alert candidates", status?.alert_candidates.length ?? 0]].map(([label, value]) => <div key={String(label)} className="rounded-lg border border-[color:var(--shell-border)] p-3"><div className="text-[10px] uppercase tracking-[0.18em] text-[color:var(--shell-muted)]">{label}</div><div className="mt-1 text-xl font-semibold text-[color:var(--shell-ink)]">{value}</div></div>)}
      </div>
