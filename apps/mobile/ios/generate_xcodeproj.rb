...
watch_extension_bundle_id = ENV.fetch("WATCH_EXTENSION_BUNDLE_ID", "#{watch_bundle_id}.watchkitextension")
...
watch_extension_target = project.new_target(:watch2_extension, "Claritas Watch App Extension", :watchos, watchos_deployment_target)
...
[ios_target, watch_target, watch_extension_target].each do |target|
...
configure_target(
  ...
  {
    "CLARITAS_WATCH_APP_BUNDLE_IDENTIFIER" => watch_bundle_id,
    "CODE_SIGN_STYLE" => "Automatic",
    "CURRENT_PROJECT_VERSION" => build_number,
    "DEVELOPMENT_TEAM" => development_team,
    "GENERATE_INFOPLIST_FILE" => "NO",
    "INFOPLIST_FILE" => "../ClaritasWatch/ExtensionInfo.plist",
    "MARKETING_VERSION" => marketing_version,
...
watch_resource_paths = ["Assets.xcassets"]
watch_extension_resource_paths = ["Config.plist"]
watch_refs = add_files(watch_group, WATCH_SOURCE_ROOT, watch_swift_paths + watch_resource_paths + watch_extension_resource_paths + ["Info.plist", "ExtensionInfo.plist"])
...
watch_swift_paths.each { |path| watch_extension_target.source_build_phase.add_file_reference(watch_refs.fetch(path)) }
["Models/Models.swift", "Services/APIClient.swift"].each do |path|
  watch_extension_target.source_build_phase.add_file_reference(ios_refs.fetch(path))
end
watch_extension_resource_paths.each { |path| watch_extension_target.resources_build_phase.add_file_reference(watch_refs.fetch(path)) }

watch_target.add_dependency(watch_extension_target)
embed_watch_extension = watch_target.new_copy_files_build_phase("Embed App Extensions")
embed_watch_extension.dst_subfolder_spec = "13"
watch_extension_product = embed_watch_extension.add_file_reference(watch_extension_target.product_reference)
watch_extension_product.settings = { "ATTRIBUTES" => ["RemoveHeadersOnCopy", "CodeSignOnCopy"] }
...
project.root_object.attributes["TargetAttributes"] = {
  ...
  watch_extension_target.uuid => { "DevelopmentTeam" => development_team, "ProvisioningStyle" => "Automatic" }
}
...
puts "watchOS extension target: #{watch_extension_bundle_id}"
