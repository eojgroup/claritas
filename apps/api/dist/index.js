...
const briefing_generator_1 = require("./briefing-generator");
const llm_1 = require("./llm");
const email_1 = require("./email");
const personal_briefing_1 = require("./personal-briefing");
const app = (0, express_1.default)();
const PORT = process.env.PORT ? Number(process.env.PORT) : 8080;
const DAILY_BRIEFING_SCHEDULER_LOCK_NAMESPACE = 9433;
const DAILY_BRIEFING_SCHEDULER_LOCK_KEY = 1;
const DAILY_BRIEFING_SCHEDULER_POLL_SECONDS = parseBoundedIntEnv(process.env.DAILY_BRIEFING_SCHEDULER_POLL_SECONDS, 30, 3600, 60);
...
let dailyBriefingSchedulerTimer = null;
...
function parseBoundedIntEnv(raw, min, max, fallback) {
...
function parsePodcastIngestParams(raw) {
    ...
    return (0, podcastindex_1.podcastParamsFromEnv)({
        feedIds,
        searchTerms,
...
function podcastErrorStatus(message) {
...
function parseGeneratedBriefingStatus(value) {
...
async function getDailySignalBriefingById(id) {
...
function toDailyBriefingGenerationJob(row, briefing = null) {
    return {
        id: row.id,
        briefing_date: dateToApiString(row.briefing_date),
        status: row.status,
...
function optionsFromGenerationJob(row) {
...
function describeGenerationError(error) {
    if (error instanceof AdminApiError ||
        error instanceof briefing_generator_1.BriefingGenerationError ||
        error instanceof llm_1.LlmConfigurationError ||
        error instanceof llm_1.LlmProviderError) {
        return error.message;
    }
    return error instanceof Error ? error.message : String(error);
}
async function createDailyBriefingGenerationJob(briefingDate, options) {
    const id = (0, crypto_1.randomUUID)();
    const { rows } = await (0, db_1.query)(`INSERT INTO daily_signal_briefing_generation_job (
       id,
       briefing_date,
       status,
       options
     ...
     RETURNING
       ...
       started_at,
       finished_at,
       updated_at`, [id, briefingDate, JSON.stringify(options)]);
    if (!rows[0])
        throw new AdminApiError(500, "Failed to queue daily briefing generation.");
    return toDailyBriefingGenerationJob(rows[0]);
}
async function getDailyBriefingGenerationJob(jobId) {
    const { rows } = await (0, db_1.query)(`SELECT
       id,
       briefing_date,
       status,
...
async function runDailyBriefingGenerationJob(jobId) {
    const { rows } = await (0, db_1.query)(`UPDATE daily_signal_briefing_generation_job
     SET status = 'running',
    ...
    try {
        ...
        const generated = await (0, briefing_generator_1.generateDailySignalBriefing)(options);
        const briefing = await upsertDailySignalBriefing(options.briefingDate, generated.payload);
        await (0, db_1.query)(`UPDATE daily_signal_briefing_generation_job
       SET status = 'success',
           briefing_id = $2,
           generation = $3::jsonb,
           error = NULL,
           finished_at = now(),
           updated_at = now()
       WHERE id = $1`, [jobId, briefing.id, JSON.stringify(generated.generation)]);
    }
    catch (error) {
...
function normalizeBriefingScheduleTimezone(value) {
    if (typeof value !== "string" || !value.trim()) {
...
function normalizeBriefingSelectionList(value, fieldName, options) {
    ...
    const values = value.map((entry) => {
        ...
        if (!trimmed || trimmed.length > options.maxLength) {
            throw new AdminApiError(400, `${fieldName} values must be between 1 and ${options.maxLength} characters.`);
        }
...
function parseBriefingSchedulePatch(raw) {
    ...
    if (Object.prototype.hasOwnProperty.call(body, "company_symbols")) {
        patch.company_symbols = normalizeBriefingSelectionList(body.company_symbols, "company_symbols", {
            maxItems: 50,
            maxLength: 16,
            uppercase: true,
...
function toDailyBriefingSchedule(row) {
    return {
...
async function ensureUserDailyBriefingSchedule(userId) {
    await (0, db_1.query)(`INSERT INTO user_daily_briefing_schedule (user_id)
     VALUES ($1)
     ON CONFLICT (user_id) DO NOTHING`, [userId]);
}
async function getUserDailyBriefingSchedule(userId) {
    ...
    const { rows } = await (0, db_1.query)(`SELECT
     ...
     LIMIT 1`, [userId]);
    if (!rows[0])
        throw new AdminApiError(500, "Failed to load daily briefing schedule.");
    return toDailyBriefingSchedule(rows[0]);
}
async function updateUserDailyBriefingSchedule(userId, patch) {
    await ensureUserDailyBriefingSchedule(userId);
    const { rows } = await (0, db_1.query)(`UPDATE user_daily_briefing_schedule
    ...
    if (!rows[0])
        throw new AdminApiError(500, "Failed to update daily briefing schedule.");
    return toDailyBriefingSchedule(rows[0]);
}
function getUtcBriefingDateForSchedule(localScheduleDate) {
    const utcToday = new Date().toISOString().slice(0, 10);
    return localScheduleDate > utcToday ? utcToday : localScheduleDate;
}
async function getDueDailyBriefingSchedules() {
    const { rows } = await (0, db_1.query)(`SELECT
       user_id,
       enabled,
       scheduled_time::text AS scheduled_time,
     ...
     LIMIT $1`, [DAILY_BRIEFING_SCHEDULER_BATCH_SIZE]);
    return rows;
}
async function getActiveDailyBriefingGenerationJob(briefingDate) {
    const { rows } = await (0, db_1.query)(`SELECT id, briefing_date, status, options, briefing_id, generation, error,
            created_at, started_at, finished_at, updated_at
     FROM daily_signal_briefing_generation_job
     WHERE briefing_date = $1::date
       AND status IN ('queued', 'running')
     ORDER BY created_at DESC
     LIMIT 1`, [briefingDate]);
    return rows[0] ?? null;
}
async function ensurePublishedDailyBriefingGenerationJob(briefingDate) {
    const published = await (0, db_1.query)(`SELECT id FROM daily_signal_briefing
    ...
    if (activeJob) {
        if (activeJob.status === "queued")
            startDailyBriefingGenerationJob(activeJob.id);
        return activeJob.id;
    }
...
async function markDailyBriefingSchedulesTriggered(userIds, localScheduleDate, jobId) {
    if (userIds.length === 0)
        return;
    await (0, db_1.query)(`UPDATE user_daily_briefing_schedule
     SET last_scheduled_for = $2::date,
         last_triggered_at = now(),
         last_job_id = COALESCE($3, last_job_id),
         updated_at = now()
     WHERE user_id = ANY($1::bigint[])`, [userIds, localScheduleDate, jobId]);
}
async function withDailyBriefingSchedulerLock(task) {
    await (0, db_1.withTransaction)(async (client) => {
        const { rows } = await client.query(`SELECT pg_try_advisory_xact_lock($1::int, $2::int) AS locked`, [DAILY_BRIEFING_SCHEDULER_LOCK_NAMESPACE, DAILY_BRIEFING_SCHEDULER_LOCK_KEY]);
        if (!rows[0]?.locked)
            return;
        await task();
    });
}
async function runDailyBriefingSchedulerCycle() {
    if (dailyBriefingSchedulerRunning)
        return;
    ...
    try {
        await withDailyBriefingSchedulerLock(async () => {
            ...
            await (0, personal_briefing_1.enqueueDuePersonalBriefingJobs)(DAILY_BRIEFING_SCHEDULER_BATCH_SIZE);
        });
    }
    finally {
...
function parseOptionalPositiveInt(value, fieldName, max) {
...
function parseBriefingGenerationOptions(briefingDate, raw) {
    const body = raw && typeof raw === "object" && !Array.isArray(raw)
        ? raw
        : {};
    const status = typeof body.publish === "boolean"
        ? body.publish
            ? "published"
            : "draft"
...
app.get("/api/briefings/daily/preferences/options", requireSession, async (_req, res) => {
    try {
        const options = await (0, personal_briefing_1.getPersonalBriefingReferenceOptions)();
...
app.get("/api/briefings/daily/email/status", requireSession, async (_req, res) => {
    try {
        const userId = getAuthenticatedUserId(res);
        const { rows } = await (0, db_1.query)(`SELECT email, email_verified FROM app_user WHERE id = $1 LIMIT 1`, [userId]);
        const emailConfig = (0, email_1.getEmailRuntimeConfig)();
        return res.json({
            email: {
...
function verificationRedirect(verified) {
    const base = (0, email_1.getEmailRuntimeConfig)().public_base_url;
    return `${base || ""}/?email_verification=${verified ? "success" : "invalid"}`;
}
app.post("/api/email-verifications", requireSession, async (_req, res) => {
    try {
        const userId = getAuthenticatedUserId(res);
        const { rows } = await (0, db_1.query)(`SELECT u.email, u.email_verified, MAX(t.created_at) AS last_requested_at
       ...
       WHERE u.id = $1 GROUP BY u.id`, [userId]);
        ...
        if (!user || !validEmail(user.email))
            return res.status(400).json({ error: "Your account has no valid email address." });
        ...
        if (!emailConfig.public_base_url)
            return res.status(503).json({ error: "EMAIL_PUBLIC_BASE_URL is not configured." });
        const token = (0, crypto_1.randomBytes)(32).toString("base64url");
        const tokenHash = (0, crypto_1.createHash)("sha256").update(token).digest("hex");
        await (0, db_1.query)(`UPDATE email_verification_token SET consumed_at = now() WHERE user_id = $1 AND consumed_at IS NULL`, [userId]);
        await (0, db_1.query)(`INSERT INTO email_verification_token (user_id, email, token_hash, expires_at) VALUES ($1, $2, $3, now() + interval '1 hour')`, [userId, user.email, tokenHash]);
        await (0, email_1.sendEmailVerificationEmail)(user.email, `${emailConfig.public_base_url}/api/email-verifications/confirm?token=${encodeURIComponent(token)}`);
...
app.get("/api/email-verifications/confirm", async (req, res) => {
    ...
    if (!token)
        return res.redirect(303, verificationRedirect(false));
    try {
        const tokenHash = (0, crypto_1.createHash)("sha256").update(token).digest("hex");
        const updated = await (0, db_1.withTransaction)(async (client) => {
            const { rows } = await client.query(`UPDATE email_verification_token SET consumed_at = now()
...
app.get("/api/briefings/daily/personal/latest", requireSession, async (_req, res) => {
    try {
        const userId = getAuthenticatedUserId(res);
        const briefing = await (0, personal_briefing_1.getLatestPersonalBriefing)(userId);
...
app.post("/api/briefings/daily/personal/preview", requireAuthenticated, async (_req, res) => {
    try {
        const userId = getAuthenticatedUserId(res);
        await ensureUserDailyBriefingSchedule(userId);
        const { rows } = await (0, db_1.query)(`SELECT timezone(schedule_timezone, now())::date AS briefing_date
       FROM user_daily_briefing_schedule
       WHERE user_id = $1
       LIMIT 1`, [userId]);
        const briefingDate = dateToApiString(rows[0]?.briefing_date ?? new Date());
        const job = await (0, personal_briefing_1.enqueuePersonalBriefingJob)(userId, briefingDate, {
            deliveryRequested: true,
            force: true,
        });
        void (0, personal_briefing_1.processPersonalBriefingJob)(job.id).catch((error) => {
...
app.get("/api/briefings/daily/personal/jobs/:id", requireSession, async (req, res) => {
    try {
        const userId = getAuthenticatedUserId(res);
        const job = await (0, personal_briefing_1.getPersonalBriefingJob)(userId, req.params.id);
        if (!job)
            return res.status(404).json({ error: "Personal briefing job not found." });
        return res.json({ job });
    }
    catch (e) {
        return res.status(500).json({ error: e.message || String(e) });
...
app.get("/api/admin/briefings/daily/generation/config", requireAdminRole, async (_req, res) => {
    try {
        return res.json({ generator: (0, briefing_generator_1.getDailyBriefingGeneratorConfig)() });
    }
    catch (e) {
        return res.status(500).json({ error: e.message || String(e) });
    }
...
app.get("/api/admin/briefings/daily/generation/jobs/:jobId", requireAdminRole, async (req, res) => {
    try {
        ...
        if (!jobId)
            throw new AdminApiError(400, "jobId is required.");
        const job = await getDailyBriefingGenerationJob(jobId);
        if (!job)
            return res.status(404).json({ error: "Daily briefing generation job not found." });
...
app.post("/api/admin/briefings/daily/:date/generate", requireAdminRole, async (req, res) => {
    try {
        ...
        if (e instanceof AdminApiError)
            return res.status(e.status).json({ error: e.message });
        if (e instanceof briefing_generator_1.BriefingGenerationError)
            return res.status(e.status).json({ error: e.message });
        if (e instanceof llm_1.LlmConfigurationError)
            return res.status(503).json({ error: e.message });
        if (e instanceof llm_1.LlmProviderError)
...
app.post("/api/ingest/briefings/daily/:date/generate", requireIngestionAccess, async (req, res) => {
    try {
        const briefingDate = parseBriefingDate(req.params.date);
        const generated = await (0, briefing_generator_1.generateDailySignalBriefing)(parseBriefingGenerationOptions(briefingDate, req.body));
...
app.get("/api/news", requireAuthenticated, async (req, res) => {
    try {
        ...
        const params = [];
        const where = ["i.kind = 'news_article'"];
        if (q) {
...
app.get("/api/news/coverage", requireAuthenticated, async (req, res) => {
    try {
        ...
        const { rows } = await (0, db_1.query)(`SELECT COALESCE(i.language_code, 'unknown') AS language,
       ...
       ORDER BY article_count DESC`, [days]);
...
app.get("/api/news/events", requireAuthenticated, async (req, res) => {
    try {
        (0, ingestion_automation_1.trackDemandSignal)("news");
        const events = await (0, gdelt_1.getGdeltEvents)({
...
app.get("/api/news/signals", requireAuthenticated, async (req, res) => {
    try {
        (0, ingestion_automation_1.trackDemandSignal)("news");
        const signals = await (0, gdelt_1.getGdeltSignals)({
            country: typeof req.query.country === "string" ? req.query.country : undefined,
...
app.get("/api/podcasts", requireAuthenticated, async (req, res) => {
    try {
        ...
        (0, ingestion_automation_1.trackDemandSignal)("podcasts");
        const { rows } = await (0, db_1.query)(`SELECT
       ...
       LIMIT $1 OFFSET $2`, [limit, offset, q, signalType]);
...
app.get("/api/podcasts/:itemId/evidence", requireAuthenticated, async (req, res) => {
    try {
        ...
        if (!Number.isSafeInteger(itemId) || itemId <= 0)
            return res.status(400).json({ error: "Invalid podcast item id." });
        const { rows } = await (0, db_1.query)(`SELECT es.id, es.segment_index, es.start_ms, es.end_ms, es.speaker, es.text,
       ...
       WHERE pe.item_id = $1
       GROUP BY es.id
       ORDER BY es.segment_index`, [itemId]);
...
app.get("/api/intelligence/entities/:name/evidence", requireAuthenticated, async (req, res) => {
    try {
        ...
        const { rows } = await (0, db_1.query)(`SELECT i.id AS item_id, i.title AS episode_title, i.event_time,
...
app.get("/api/podcasts/discover", requireAdminRole, async (req, res) => {
    try {
        ...
        const feeds = await (0, podcastindex_1.discoverPodcastFeeds)(q, Math.min(Math.max(Number(req.query.limit) || 20, 1), 50));
...
app.post("/api/ingest/podcastindex/episodes", requireIngestionAccess, async (req, res) => {
    try {
        return res.json(await (0, podcastindex_1.ingestPodcastIndex)(parsePodcastIngestParams(req.body)));
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        return res.status(podcastErrorStatus(message)).json({ error: message });
...
app.post("/api/admin/ingestion/podcasts/run", requireAdminRole, async (req, res) => {
    try {
        const plan = (0, ingestion_admin_1.buildPodcastRunPlan)(req.body);
        const { runId } = await (0, ingestion_admin_1.triggerPodcastRun)({ actor: getRequestActor(res), plan });
        const detail = await (0, ingestion_admin_1.getRunDetail)(runId, 150);
        if (!detail)
        ...
        const message = error instanceof Error ? error.message : String(error);
        return res.status(error instanceof ingestion_admin_1.IngestionValidationError ? 400 : 500).json({ error: message });
    }
});
app.post("/api/admin/ingestion/leadership/run", requireAdminRole, async (req, res) => {
    try {
        const plan = (0, ingestion_admin_1.buildLeadershipRunPlan)(req.body);
        const { runId } = await (0, ingestion_admin_1.triggerLeadershipRun)({ actor: getRequestActor(res), plan });
        const detail = await (0, ingestion_admin_1.getRunDetail)(runId, 150);
        if (!detail)
            return res.status(500).json({ error: "Failed to load created run." });
        return res.status(202).json(detail);
    ...
    try {
        ...
        const [statsResult, coverageResult] = await Promise.all([
         ...
         WHERE kind = 'news_article'
           AND COALESCE(event_time, created_at) >= now() - ($1 || ' days')::interval`, params),
        ]);
...
app.post("/api/ingest/wikidata/leadership", requireIngestionAccess, async (_req, res) => {
    try {
        return res.json(await (0, wikidata_leadership_1.ingestWikidataLeadership)());
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
...
app.post("/api/ingest/gdelt", requireIngestionAccess, async (req, res) => {
    try {
        ...
        const result = await (0, gdelt_1.ingestGdelt)({
...
app.post("/api/ingest/openmeteo/country-weather", requireIngestionAccess, async (req, res) => {
    try {
        ...
        return res.json(await (0, openmeteo_1.ingestOpenMeteoCountryWeather)(country));
    }
    catch (error) {
        return res.status(502).json({ error: error instanceof Error ? error.message : String(error) });
    }
...
app.post("/api/ingest/ecb", requireIngestionAccess, async (req, res) => {
    try {
        const currencies = Array.isArray(req.body?.currencies)
            ? req.body.currencies.filter((value) => typeof value === "string")
            : typeof req.body?.currencies === "string" ? req.body.currencies.split(/[\s,]+/).filter(Boolean) : undefined;
        return res.json(await (0, ecb_1.ingestEcbData)({
            currencies,
...
app.get("/api/weather/country-latest", requireAuthenticated, async (_req, res) => {
    try {
        ...
        const rows = await (0, openmeteo_1.getCountryWeatherLatest)();
        res.json({ stats: rows });
    }
    catch (e) {
        res.status(500).json({ error: e.message || String(e) });
...
app.get("/api/weather/forecast", requireAuthenticated, async (req, res) => {
    try {
        ...
        const forecast = await (0, openmeteo_1.getCountryWeatherForecast)(country, Number.isFinite(hours) ? hours : 48);
        if (!forecast)
            return res.status(404).json({ error: "No forecast is available for this country." });
...
app.get("/api/weather/history", requireAuthenticated, async (req, res) => {
    try {
        ...
        const history = await (0, openmeteo_1.getHistoricalWeather)(country, typeof req.query.start_date === "string" ? req.query.start_date : defaultStart, typeof req.query.end_date === "string" ? req.query.end_date : defaultEnd);
        if (!history)
            return res.status(404).json({ error: "Historical weather is unavailable for this country." });
...
app.get("/api/weather/marine", requireAuthenticated, async (req, res) => {
    try {
        ...
        if (!/^[A-Z]{2}$/.test(country))
            return res.status(400).json({ error: "country must be an ISO alpha-2 code." });
        const hours = typeof req.query.hours === "string" ? Number(req.query.hours) : 48;
        const marine = await (0, openmeteo_1.getMarineWeather)(country, Number.isFinite(hours) ? hours : 48);
        if (!marine)
            return res.status(404).json({ error: "Marine weather is unavailable for this country." });
...
app.get("/api/leadership/countries", requireAuthenticated, async (_req, res) => {
    try {
        (0, ingestion_automation_1.trackDemandSignal)("leadership");
        const countries = await (0, wikidata_leadership_1.getCountryLeadershipLatest)();
        res.json({ countries });
    }
    catch (e) {
        res.status(500).json({ error: e.message || String(e) });
...
app.get("/api/market/filings", requireAuthenticated, async (req, res) => {
    try {
        ...
        const filings = await (0, sec_edgar_1.getMarketFilings)({
...
app.get("/api/market/indicators", requireAuthenticated, async (req, res) => {
    try {
        (0, ingestion_automation_1.trackDemandSignal)("market");
        const indicators = await (0, sec_edgar_1.getMarketIndicators)({
...
app.get("/api/market/fx", requireAuthenticated, async (_req, res) => {
    try {
        (0, ingestion_automation_1.trackDemandSignal)("market");
        const rates = await (0, ecb_1.getLatestFxRates)();
...
app.get("/api/market/rates", requireAuthenticated, async (_req, res) => {
    try {
        (0, ingestion_automation_1.trackDemandSignal)("market");
        const rates = await (0, ecb_1.getLatestPolicyRates)();
        return res.json({ rates, count: rates.length, attribution: "European Central Bank" });
    }
    catch (error) {
        return res.status(500).json({ error: error instanceof Error ? error.message : String(error) });
