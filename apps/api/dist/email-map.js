...
function countryFill(relevance, featured) {
    if (featured)
        return "#9a6038";
    if (relevance <= 0)
        return "#1b2d38";
    ...
    const rgb = low.map((channel, index) => Math.round(channel + (high[index] - channel) * ratio));
    return `rgb(${rgb.join(",")})`;
}
function renderBriefingMapSvg(countries) {
    ...
    const paths = WORLD_FEATURES.map((countryFeature) => {
        ...
        return `<path d="${geometryPath(countryFeature.geometry)}" fill="${countryFill(datum?.relevance_score ?? 0, isFeatured)}" stroke="${isFeatured ? "#f8dfc9" : "#3d5562"}" stroke-width="${isFeatured ? 2.2 : 0.8}" fill-rule="evenodd"/>`;
    }).join("");
    const markers = ranked.flatMap((country, index) => {
        ...
        const label = labelVisible
            ? `<text x="${x.toFixed(1)}" y="${(y - radius - 8).toFixed(1)}" text-anchor="middle" fill="#f7efe8" stroke="#07121a" stroke-width="5" paint-order="stroke" font-size="18" font-weight="700" letter-spacing="1">${escapeXml(country.country_iso2)}</text>`
            : "";
        const featuredRing = index === 0
            ? `<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${(radius + 9).toFixed(1)}" fill="none" stroke="#ffd7b5" stroke-width="3" stroke-dasharray="7 5"/>`
            : "";
        return [
            `<g>${featuredRing}<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${(radius + 7).toFixed(1)}" fill="#f0a66f" fill-opacity="0.22"/><circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${radius.toFixed(1)}" fill="#f0a66f" stroke="#fff0d9" stroke-width="2.5"/><circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${(radius * 0.52).toFixed(1)}" fill="none" stroke="#ffffff" stroke-opacity="0.5" stroke-width="2"/>${label}</g>`,
        ];
    }).join("");
    const featuredText = featured
       ...
       <text x="36" y="62" fill="#f7efe8" font-size="23" font-weight="700">${escapeXml(featured.country_name)} · ${escapeXml(featured.country_iso2)}</text>
       <text x="${WIDTH - 36}" y="52" text-anchor="end" fill="#9fb0ba" font-size="16">Relevance ${Math.round(featured.relevance_score)}/100</text>`
...
async function renderBriefingMapPng(countries) {
    return (0, sharp_1.default)(Buffer.from(renderBriefingMapSvg(countries)))
