...
function renderBriefingMapSvg(countries, theme = "dark") {
    ...
    const markers = ranked.flatMap((country, index) => {
        ...
        const featuredRing = index === 0
            ...
            : "";
        return [
            `<g>${featuredRing}<circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${(radius + 7).toFixed(1)}" fill="${palette.markerHalo}" fill-opacity="0.22"/><circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${radius.toFixed(1)}" fill="${palette.marker}" stroke="${palette.markerStroke}" stroke-width="2.5"/><circle cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${(radius * 0.52).toFixed(1)}" fill="none" stroke="${palette.markerStroke}" stroke-opacity="0.55" stroke-width="2"/>${label}</g>`,
        ];
    }).join("");
