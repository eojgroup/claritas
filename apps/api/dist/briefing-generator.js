...
exports.BriefingGenerationError = void 0;
exports.generateDailySignalBriefing = generateDailySignalBriefing;
...
function getSourceWindow(briefingDate, lookbackHours) {
    ...
    const boundedLookbackHours = Number.isFinite(lookbackHours) && lookbackHours > 0
        ? Math.min(Math.max(Math.trunc(lookbackHours), 1), 168)
        : null;
...
async function collectBriefingContext(options) {
    ...
    const [newsResult, gdeltEventResult, gdeltSignalResult, podcastResult, marketOverview, marketInstrumentSnapshots, marketEventResult, weatherResult, leadershipResult, transportResult,] = await Promise.all([
        (0, db_1.query)(`SELECT
         i.id,
       ...
       LIMIT $3`, [start, end, podcastLimit]),
        ...
        (0, market_instruments_1.getMarketInstrumentSnapshots)({ limit: Math.min(marketLimit * 3, 120) }),
        (0, db_1.query)(`SELECT
         me.event_type,
         me.symbol,
         me.company_name,
...
function buildUserPrompt(context, instructions) {
...
function normalizeModelOutput(output, context) {
    const title = sanitizeText(output.title, 120) || "Daily signal brief";
    const updateText = sanitizeText(output.update_text, 1800) ||
    ...
    return {
        ...
        key_takeaways: takeaways.length > 0
            ? takeaways
...
async function generateDailySignalBriefing(options, llmClient = (0, llm_1.createLlmClientFromEnv)()) {
...
function getDailyBriefingGeneratorConfig() {
    return {
        prompt_version: PROMPT_VERSION,
        llm: (0, llm_1.getLlmRuntimeConfig)(),
    };
}
