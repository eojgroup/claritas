...
exports.extractPodcastIntelligence = extractPodcastIntelligence;
...
function canonicalKey(value) {
    ...
    return normalized || node_crypto_1.default.createHash("sha256").update(value).digest("hex").slice(0, 32);
}
...
function normalizeSignal(raw) {
    ...
    if (!type || !SIGNAL_TYPES.has(type) || !title)
        return null;
...
function metadataSignals(persons, categories) {
    const entitySignals = persons.flatMap((person) => {
        ...
        return [{
                ...
                evidence: [], method: "podcast:person",
    ...
    const topicSignals = Object.values(categories || {}).flatMap((category) => {
        ...
        return [{
                ...
                entities: [], topics: [topic], risk_level: null, confidence: 0.8,
                evidence: [], method: "podcast:category", metadata: {},
            }];
    });
    return [...entitySignals, ...topicSignals];
}
async function runLlmExtraction(episode, segments) {
    if (!extractionEnabled() || segments.length === 0)
        return [];
    ...
    const response = await (0, llm_1.createLlmClientFromEnv)().generateStructured({
    ...
    return signals.map((signal) => normalizeSignal(signal)).filter((signal) => Boolean(signal));
}
async function extractPodcastIntelligence(episodeId, persons, categories) {
    const [episodeResult, segmentResult] = await Promise.all([
        (0, db_1.query)(`SELECT i.title, i.summary, pf.title AS feed_title
       FROM podcast_episode pe
       JOIN item i ON i.id = pe.item_id
       JOIN podcast_feed pf ON pf.id = pe.feed_id
       WHERE pe.id = $1`, [episodeId]),
        (0, db_1.query)(`SELECT id, segment_index, start_ms, end_ms, speaker, text
       FROM evidence_segment WHERE episode_id = $1 ORDER BY segment_index ASC`, [episodeId]),
    ]);
    const episode = episodeResult.rows[0];
    if (!episode)
        return 0;
    const segments = segmentResult.rows;
    const signalInputs = [...metadataSignals(persons, categories)];
    ...
    await (0, db_1.withTransaction)(async (client) => {
        ...
        for (const raw of signalInputs) {
            ...
            const inserted = await client.query(`INSERT INTO intelligence_signal (
         ...
         RETURNING id`, [
            ...
            for (const index of evidenceIndexes) {
                ...
                if (!segment)
                    continue;
                await client.query(`INSERT INTO intelligence_signal_evidence (signal_id, evidence_segment_id, relevance, quote)
           VALUES ($1,$2,$3,$4)
           ON CONFLICT (signal_id, evidence_segment_id) DO UPDATE SET relevance = EXCLUDED.relevance, quote = EXCLUDED.quote`, [inserted.rows[0].id, segment.id, confidence, segment.text.slice(0, 800)]);
            }
        }
       ...
       SET processed_at = now(),
           ...
           END,
           updated_at = now()
       WHERE id = $1`, [episodeId, extractionError]);
    });
    return stored;
}
