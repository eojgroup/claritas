...
exports.ingestFredMarketData = ingestFredMarketData;
const db_1 = require("../db");
const fred_market_data_1 = require("./fred-market-data");
...
function requireApiKey() {
    ...
    if (!key)
        throw new Error("FRED selected but FRED_API_KEY is not configured.");
    return key;
}
async function fetchJson(url) {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(`FRED API HTTP ${response.status}: ${(await response.text()).slice(0, 240)}`);
...
async function ensureSource() {
    const { rows } = await (0, db_1.query)(`INSERT INTO source (name,api_base_url,auth_type,metadata)
     ...
     RETURNING id`, [FRED_API, JSON.stringify({
...
async function ensureUsCountry() {
    await (0, db_1.query)(`INSERT INTO country (iso2,iso3,name,region) VALUES ('US','USA','United States','Americas')
     ON CONFLICT (iso2) DO UPDATE SET iso3=COALESCE(country.iso3,EXCLUDED.iso3),
       region=COALESCE(country.region,EXCLUDED.region)`);
}
async function ensureInstrument(sourceId, definition) {
    const { rows } = await (0, db_1.query)(`INSERT INTO market_instrument (
     ...
     RETURNING id`, [sourceId, definition.seriesId, definition.canonicalSymbol, definition.name,
    ...
    await (0, db_1.query)(`INSERT INTO market_instrument_country (instrument_id,country_iso2,relationship,is_primary,metadata)
     VALUES ($1,$2,$3,$4,$5::jsonb)
     ON CONFLICT (instrument_id,country_iso2,relationship) DO UPDATE SET
       is_primary=EXCLUDED.is_primary,metadata=EXCLUDED.metadata`, [instrumentId, linkCountry, definition.relationship, definition.country != null,
        JSON.stringify({ original_publisher: definition.originalPublisher })]);
    return instrumentId;
}
function defaultStart(definition) {
    const date = new Date();
    date.setUTCFullYear(date.getUTCFullYear() - (definition.frequency === "daily" ? 3 : 12));
    return date.toISOString().slice(0, 10);
}
async function ingestFredMarketData() {
    ...
    let failed = 0;
    const results = [];
    for (const definition of fred_market_data_1.FRED_SERIES) {
        try {
            const instrumentId = await ensureInstrument(sourceId, definition);
            const latest = await (0, db_1.query)(`SELECT max(period_end) AS period_end FROM market_indicator WHERE instrument_id=$1`, [instrumentId]);
            ...
            const observations = (0, fred_market_data_1.parseFredObservations)(await fetchJson(url));
           ...
           RETURNING (xmax=0) AS inserted`, [sourceId, instrumentId, `${definition.seriesId}:${observation.date}`, definition.category,
    ...
    if (failed === fred_market_data_1.FRED_SERIES.length)
        throw new Error(`All FRED series failed: ${String(results[0]?.error ?? "unknown error")}`);
    return { provider: "fred", series: fred_market_data_1.FRED_SERIES.length, inserted, updated, skipped, failed, results };
}
