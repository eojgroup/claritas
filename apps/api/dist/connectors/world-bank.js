...
exports.ingestWorldBankIndicators = ingestWorldBankIndicators;
const world_countries_1 = __importDefault(require("world-countries"));
const db_1 = require("../db");
const world_bank_market_data_1 = require("./world-bank-market-data");
...
const byIso3 = new Map(world_countries_1.default.flatMap((country) => country.cca2 && country.cca3
    ? [[country.cca3.toUpperCase(), country]] : []));
async function fetchJson(url) {
    ...
    try {
        ...
        if (!response.ok)
            throw new Error(`World Bank Indicators API HTTP ${response.status}: ${(await response.text()).slice(0, 240)}`);
...
async function ensureSource() {
    const { rows } = await (0, db_1.query)(`INSERT INTO source (name,api_base_url,auth_type,metadata)
     ...
     RETURNING id`, [API_BASE, JSON.stringify({
...
async function ensureCountry(iso2, iso3, name, region) {
    await (0, db_1.query)(`INSERT INTO country (iso2,iso3,name,region) VALUES ($1,$2,$3,$4)
     ON CONFLICT (iso2) DO UPDATE SET iso3=COALESCE(country.iso3,EXCLUDED.iso3),
       ...
       region=COALESCE(country.region,EXCLUDED.region)`, [iso2, iso3, name, region]);
}
async function ensureInstrument(sourceId, definition, iso2, countryName, indicatorName) {
    const providerSymbol = `${definition.code}:${iso2}`;
    const { rows } = await (0, db_1.query)(`INSERT INTO market_instrument (
     ...
     RETURNING id`, [sourceId, providerSymbol, `WB:${providerSymbol}`, `${countryName} · ${definition.shortName}`,
    ...
    await (0, db_1.query)(`INSERT INTO market_instrument_country (instrument_id,country_iso2,relationship,is_primary)
     VALUES ($1,$2,'economic_indicator',true)
     ON CONFLICT (instrument_id,country_iso2,relationship) DO UPDATE SET is_primary=true`, [instrumentId, iso2]);
    return instrumentId;
}
async function ingestWorldBankIndicators() {
    ...
    let failed = 0;
    const results = [];
    const ensuredCountries = new Set();
    for (const definition of world_bank_market_data_1.WORLD_BANK_INDICATORS) {
        try {
            ...
            const parsed = (0, world_bank_market_data_1.parseWorldBankResponse)(await fetchJson(url));
            const cursor = await (0, db_1.query)(`SELECT cursor->>'last_updated' AS last_updated
         FROM source_feed WHERE source_id=$1 AND feed_key=$2`, [sourceId, `indicator:${definition.code}`]);
           ...
           RETURNING (xmax=0) AS inserted`, [sourceId, instrumentId, `${providerSymbol}:${observation.year}`, providerSymbol, iso2,
            ...
            await (0, db_1.query)(`INSERT INTO source_feed (source_id,feed_key,params,cursor)
         VALUES ($1,$2,$3::jsonb,$4::jsonb)
         ON CONFLICT (source_id,feed_key) DO UPDATE SET
           params=EXCLUDED.params,cursor=EXCLUDED.cursor,updated_at=now()`, [sourceId, `indicator:${definition.code}`, JSON.stringify({ source: 2, mrv: 10 }),
    ...
    if (failed === world_bank_market_data_1.WORLD_BANK_INDICATORS.length) {
        throw new Error(`All World Bank indicator requests failed: ${String(results[0]?.error ?? "unknown error")}`);
    }
    return { provider: "world-bank", dataset: DATASET, indicators: world_bank_market_data_1.WORLD_BANK_INDICATORS.length, inserted, updated, skipped, failed, results };
}
