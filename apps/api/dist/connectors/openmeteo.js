...
exports.ingestOpenMeteoCountryWeather = ingestOpenMeteoCountryWeather;
...
exports.getMarineWeather = getMarineWeather;
...
function arrayValue(values, key, index) {
...
function recordsFromColumns(columns) {
    const times = columns?.time ?? [];
    return times.map((time, index) => Object.fromEntries(Object.entries(columns ?? {}).map(([key, values]) => [key, values[index] ?? null])));
}
...
async function ensureSource() {
    const { rows } = await (0, db_1.query)(`INSERT INTO source (name, api_base_url, auth_type, metadata)
     ...
     RETURNING id`, [forecastBase(), apiKey() ? "api_key" : "none", JSON.stringify({
...
async function ensureCountry(target) {
    const country = world_countries_1.default.find((entry) => entry.cca2 === target.iso2);
    await (0, db_1.query)(`INSERT INTO country (iso2, iso3, name, region, centroid, ext)
     ...
     ON CONFLICT (iso2) DO UPDATE SET
       ...
       ext = COALESCE(country.ext, '{}'::jsonb) || EXCLUDED.ext`, [target.iso2, country?.cca3 ?? null, target.name, country?.region ?? null,
        `${target.lat},${target.lon}`, target.lat, target.lon]);
}
async function weatherTargets(countryIso2) {
    ...
    if (requested) {
        ...
        const { rows } = await (0, db_1.query)(`SELECT iso2::text FROM country ORDER BY iso2`);
        iso2s = rows.map((row) => row.iso2.trim().toUpperCase());
    }
    return Array.from(new Set(iso2s)).map((iso2) => {
        const country = world_countries_1.default.find((entry) => entry.cca2 === iso2);
        if (!country || !Array.isArray(country.latlng) || country.latlng.length < 2)
            return null;
        return { iso2, name: country.name.common, lat: country.latlng[0], lon: country.latlng[1] };
    }).filter((target) => Boolean(target));
}
function addCoordinates(url, targets) {
...
async function fetchForecast(targets) {
    ...
    if (!response.ok)
        throw new Error(`Open-Meteo forecast HTTP ${response.status}: ${(await response.text()).slice(0, 300)}`);
    const data = await response.json();
    return Array.isArray(data) ? data : [data];
}
async function fetchAirQuality(targets) {
    ...
    const response = await fetch(url, { headers: { accept: "application/json" } });
    if (!response.ok)
        throw new Error(`Open-Meteo air-quality HTTP ${response.status}: ${(await response.text()).slice(0, 300)}`);
    const data = await response.json();
    return Array.isArray(data) ? data : [data];
}
function responseForIndex(responses, index) {
    return responses.find((response, responseIndex) => (response.location_id ?? responseIndex) === index) ?? responses[index];
}
async function upsertTargetWeather(sourceId, target, data, air) {
    ...
    await (0, db_1.withTransaction)(async (client) => {
       ...
       ON CONFLICT (source_id, country_iso2) DO UPDATE SET
         ...
         is_day = EXCLUDED.is_day, source_kind = EXCLUDED.source_kind, updated_at = now()`, [sourceId, target.iso2, data.latitude ?? target.lat, data.longitude ?? target.lon,
        ...
        for (let index = 0; index < hourlyTimes.length; index += 1) {
         ...
         ON CONFLICT (source_id, country_iso2, granularity, forecast_time) DO UPDATE SET
           ...
           payload = EXCLUDED.payload, updated_at = now()`, [sourceId, target.iso2, forecastTime, asNumber(arrayValue(hourly, "temperature_2m", index)),
        ...
        for (let index = 0; index < dailyTimes.length; index += 1) {
         ...
         ON CONFLICT (source_id, country_iso2, granularity, forecast_time) DO UPDATE SET
           ...
           sunset_at = EXCLUDED.sunset_at, payload = EXCLUDED.payload, updated_at = now()`, [sourceId, target.iso2, forecastTime, asNumber(arrayValue(daily, "temperature_2m_min", index)),
        ...
        if (air?.current) {
         ...
         ON CONFLICT (source_id, country_iso2) DO UPDATE SET
           ...
           payload = EXCLUDED.payload, updated_at = now()`, [sourceId, target.iso2, airTime, asNumber(currentAir.european_aqi), asNumber(currentAir.us_aqi),
        ...
        await client.query(`DELETE FROM weather_forecast
       WHERE source_id = $1 AND country_iso2 = $2
         AND (forecast_time < now() - interval '2 days' OR forecast_time > now() + interval '17 days')`, [sourceId, target.iso2]);
    });
}
async function ingestOpenMeteoCountryWeather(countryIso2) {
...
async function getCountryWeatherLatest() {
    ...
    return rows.map((row) => ({
        ...
        air_quality: row.air_quality && typeof row.air_quality === "object" ? row.air_quality : null,
    }));
}
async function getCountryWeatherForecast(countryIso2, hours = 48) {
    ...
    const [hourly, daily, air] = await Promise.all([
        (0, db_1.query)(`SELECT forecast_time, temp_c, apparent_temp_c, humidity, precipitation_probability,
              precipitation_mm, rain_mm, snowfall_cm, weather_code, wind_speed, wind_gust,
       ...
       ORDER BY CASE s.name WHEN 'openmeteo' THEN 0 ELSE 1 END, wf.forecast_time LIMIT $2`, [country, hourLimit]),
        (0, db_1.query)(`SELECT wf.forecast_time, wf.temp_min_c, wf.temp_max_c, wf.apparent_temp_min_c,
       ...
       ORDER BY CASE s.name WHEN 'openmeteo' THEN 0 ELSE 1 END, wf.forecast_time LIMIT 16`, [country]),
        (0, db_1.query)(`SELECT a.observed_at, a.european_aqi, a.us_aqi, a.pm10, a.pm2_5,
    ...
    if (hourly.rows.length === 0 && daily.rows.length === 0)
        return null;
...
async function getHistoricalWeather(countryIso2, startDate, endDate) {
    ...
    const response = await fetch(url, { headers: { accept: "application/json" } });
    if (!response.ok)
        throw new Error(`Open-Meteo archive HTTP ${response.status}: ${(await response.text()).slice(0, 300)}`);
    const data = await response.json();
...
async function getMarineWeather(countryIso2, hours = 48) {
    const target = (await weatherTargets(countryIso2))[0];
    if (!target)
        return null;
    ...
    const response = await fetch(url, { headers: { accept: "application/json" } });
    if (!response.ok)
        throw new Error(`Open-Meteo marine HTTP ${response.status}: ${(await response.text()).slice(0, 300)}`);
    const data = await response.json();
    return {
        country: target.iso2,
