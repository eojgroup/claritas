...
exports.getMarketInstrumentSnapshots = getMarketInstrumentSnapshots;
exports.getMarketInstrumentCoverage = getMarketInstrumentCoverage;
const db_1 = require("../db");
const asArray = (value) => Array.isArray(value) ? value : [];
const asObject = (value) => value && typeof value === "object" && !Array.isArray(value)
    ? value
    : {};
async function getMarketInstrumentSnapshots(params = {}) {
    ...
    const { rows } = await (0, db_1.query)(`WITH ranked AS (
     ...
     ORDER BY CASE COALESCE(latest.instrument_type,'equity_index') WHEN 'equity_index' THEN 0 WHEN 'commodity' THEN 1 ELSE 2 END,
       ...
       latest.name
     LIMIT $4`, [symbols, country, instrumentType, limit]);
    return rows.map((row) => ({
        ...row,
...
async function getMarketInstrumentCoverage() {
