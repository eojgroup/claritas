...
exports.ingestEcbData = ingestEcbData;
exports.getLatestFxRates = getLatestFxRates;
exports.getLatestPolicyRates = getLatestPolicyRates;
...
async function fetchSeries(config, startPeriod) {
...
async function ensureSource() {
    const { rows } = await (0, db_1.query)(`INSERT INTO source (name, api_base_url, auth_type, metadata)
     ...
     RETURNING id`, [ECB_API_BASE, JSON.stringify({
...
async function upsertRows(sourceId, config, rows) {
    let inserted = 0;
    let updated = 0;
    let skipped = 0;
    for (const row of rows) {
        ...
        if (!period || !/^\d{4}-\d{2}-\d{2}$/.test(period) || !Number.isFinite(value)) {
            skipped += 1;
            continue;
        }
       ...
       RETURNING (xmax = 0) AS inserted`, [sourceId, externalId, config.category, seriesKey, symbol ?? null,
...
async function ingestEcbData(params = {}) {
...
async function getLatestPolicyRates() {
    const { rows } = await (0, db_1.query)(`SELECT DISTINCT ON (mi.series_key)
