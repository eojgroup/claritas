...
exports.ingestOecdSharePrices = ingestOecdSharePrices;
...
const iso3ToIso2 = new Map(world_countries_1.default.flatMap((country) => country.cca2 && country.cca3 ? [[country.cca3.toUpperCase(), country.cca2.toUpperCase()]] : []));
...
async function ensureSource() {
    const { rows } = await (0, db_1.query)(`INSERT INTO source (name, api_base_url, auth_type, metadata)
     ...
     RETURNING id`, [OECD_API, JSON.stringify({
...
async function ensureInstrument(sourceId, iso2, name, unit) {
    ...
    const { rows } = await (0, db_1.query)(`INSERT INTO market_instrument (
     ...
     RETURNING id`, [sourceId, providerSymbol, `OECD-SHARE-${iso2}`, name, unit, iso2, JSON.stringify({
    ...
    await (0, db_1.query)(`INSERT INTO market_instrument_country (instrument_id,country_iso2,relationship,is_primary)
     VALUES ($1,$2,'primary_market',true) ON CONFLICT DO NOTHING`, [instrumentId, iso2]);
    return instrumentId;
}
async function ingestOecdSharePrices() {
    ...
    for (const row of rows) {
        ...
        await (0, db_1.query)(`INSERT INTO country (iso2, iso3, name)
       VALUES ($1::char(2), $2::char(3), $3)
       ON CONFLICT (iso2) DO UPDATE SET iso3 = COALESCE(country.iso3, EXCLUDED.iso3)`, [iso2, row.REF_AREA, country?.name?.common ?? iso2]);
       ...
       RETURNING (xmax = 0) AS inserted`, [sourceId, instrumentId, externalId, `OECD:SHARE:${iso2}`, iso2,
