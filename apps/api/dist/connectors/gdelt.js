...
exports.ingestGdelt = ingestGdelt;
exports.getGdeltEvents = getGdeltEvents;
exports.getGdeltSignals = getGdeltSignals;
...
function countryNameToIso2(value) {
    const name = value?.trim().toLowerCase();
    if (!name)
        return null;
    const match = world_countries_1.default.find((country) => country.name.common.toLowerCase() === name || country.name.official.toLowerCase() === name);
    return match?.cca2 ?? null;
}
function gdeltCountryToIso2(value) {
    const code = value?.trim().toUpperCase();
    if (!code)
        return null;
    if (FIPS_TO_ISO2[code])
        return FIPS_TO_ISO2[code];
    if (code.length === 3)
        return world_countries_1.default.find((country) => country.cca3 === code)?.cca2 ?? null;
    return world_countries_1.default.some((country) => country.cca2 === code) ? code : null;
}
...
async function ensureCountry(iso2) {
    ...
    await (0, db_1.query)(`INSERT INTO country (iso2, iso3, name, region, ext)
     ...
     ON CONFLICT (iso2) DO UPDATE SET
       ...
       name = CASE WHEN country.name = country.iso2::text THEN EXCLUDED.name ELSE country.name END`, [iso2, country?.cca3 ?? null, country?.name.common ?? iso2, country?.region ?? null]);
}
async function ensureSource() {
    const { rows } = await (0, db_1.query)(`INSERT INTO source (name, api_base_url, auth_type, metadata)
     ...
     RETURNING id`, [DOC_API_URL, JSON.stringify({
            provider: "gdelt",
...
function firstZipText(bytes) {
    ...
    if (!first)
        throw new Error("GDELT archive was empty.");
    return new TextDecoder("utf-8").decode(first);
}
async function getLatestArchiveUrls() {
...
async function ingestEventArchive(sourceId, archiveUrl, maxRows) {
    ...
    for (const line of text.split(/\r?\n/)) {
        ...
        await (0, db_1.query)(`INSERT INTO global_event (
       ...
       ON CONFLICT (source_id, external_id) DO UPDATE SET
         ...
         updated_at = now()`, [sourceId, externalId, fields[26] || null, fields[28] || null, asNumber(fields[29]),
...
async function ingestGkgArchive(sourceId, archiveUrl, maxRows) {
    const response = await fetchRetry(archiveUrl);
    const text = firstZipText(new Uint8Array(await response.arrayBuffer()));
    let count = 0;
    for (const line of text.split(/\r?\n/)) {
        ...
        const primaryCountry = locations.map((location) => location.country_iso2).find((value) => typeof value === "string");
        ...
        await (0, db_1.query)(`INSERT INTO news_signal (
       ...
       ON CONFLICT (source_id, external_id) DO UPDATE SET
         ...
         event_time = EXCLUDED.event_time, payload = EXCLUDED.payload, updated_at = now()`, [sourceId, fields[0], url, domain, null, primaryCountry ?? null, toneParts[0], toneParts[1],
...
async function ingestDocArticles(sourceId, params) {
    ...
    const urls = articles.map((article) => article.url).filter((value) => Boolean(value));
    const signals = urls.length > 0
        ? await (0, db_1.query)(`SELECT DISTINCT ON (url) url, tone, themes, persons, organizations, locations, payload
    ...
    for (const article of articles) {
        ...
        const locations = Array.isArray(signal?.locations) ? signal.locations : [];
        const gkgCountry = locations.map((location) => location.country_iso2).find((value) => typeof value === "string");
        const inference = (0, country_inference_1.inferNewsCountry)({ title: article.title, url, feedCountryHint: gkgCountry ?? null });
        ...
        const dedupeHash = node_crypto_1.default.createHash("sha256").update(`${url}|${eventTime}|gdelt-doc`).digest("hex");
       ...
       RETURNING (xmax = 0) AS inserted`, [sourceId, externalId, article.title || null,
            signal ? `GDELT themes: ${signal.themes.slice(0, 4).join(", ")}` : null,
...
async function ingestGdelt(params = {}) {
    ...
    const result = { provider: "gdelt", inserted: 0, updated: 0, skipped: 0 };
    let archiveUrls = null;
    if (includeEvents || includeGkg)
        archiveUrls = await getLatestArchiveUrls();
...
async function getGdeltEvents(params = {}) {
    const country = params.country?.trim().toUpperCase() || "";
    const limit = clampInt(params.limit, 1, 500, 100);
    const { rows } = await (0, db_1.query)(`SELECT id, external_id, event_code, event_root_code, quad_class, goldstein_scale,
...
async function getGdeltSignals(params = {}) {
    const country = params.country?.trim().toUpperCase() || "";
    const theme = params.theme?.trim() || "";
    const limit = clampInt(params.limit, 1, 500, 100);
    const { rows } = await (0, db_1.query)(`SELECT id, external_id, url, domain, language_code, source_country_iso2 AS country,
            tone, positive_score, negative_score, polarity, themes, persons, organizations,
