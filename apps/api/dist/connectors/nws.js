...
exports.ingestNwsAlerts = ingestNwsAlerts;
...
async function ingestNwsAlerts() {
    ...
    if (!response.ok)
        throw new Error(`NWS alerts API HTTP ${response.status}: ${(await response.text()).slice(0, 300)}`);
    const data = await response.json();
    const sourceId = await ensureSource();
    await (0, db_1.query)(`INSERT INTO country (iso2,name) VALUES ('US','United States') ON CONFLICT (iso2) DO NOTHING`);
    let inserted = 0;
    let updated = 0;
    let skipped = 0;
    for (const feature of data.features ?? []) {
        ...
        if (!externalId || !props.event) {
            skipped += 1;
            continue;
        }
        const result = await (0, db_1.query)(`INSERT INTO weather_alert (
       ...
       RETURNING (xmax = 0) AS inserted`, [sourceId, externalId, props.senderName ?? "National Weather Service", props.event, props.severity ?? null,
