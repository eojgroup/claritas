...
export function prioritizeAdsbRouteLookups(
  lookups: AdsbRouteLookup[],
  limit: number,
  generation: number,
): AdsbRouteLookup[] {
  if (limit <= 0 || lookups.length === 0) return [];
  if (lookups.length <= limit) return [...lookups];

  const grouped = new Map<string, AdsbRouteLookup[]>();
  ...
  const ranked = Array.from(grouped, ([scope, group]) => {
    const shuffled = [...group].sort(
      (left, right) =>
        stableHash(`${generation}:${left.callsign}`) -
    ...
    .sort(
      (left, right) =>
        left.rank - right.rank || left.tieBreaker - right.tieBreaker,
    );
