...
const GAL_RELEVANCE_PATTERNS: RegExp[] = [
  ...
  /\b(?:earnings|revenue|quarterly results?|(?:annual|quarterly|record|operating|net) profits?|(?:annual|quarterly|operating|net) loss(?:es)?|(?:reports?|posts?) (?:an? )?(?:annual|quarterly|record|operating|net) profits?|(?:reports?|posts?) (?:an? )?(?:annual|quarterly|operating|net) loss(?:es)?|warns? of (?:an? )?(?:annual|quarterly|operating|net) (?:profits?|loss(?:es)?)|(?:profits?|loss(?:es)?) (?:jump|jumps|jumped|rise|rises|rose|fall|falls|fell|widen|widens|widened|narrow|narrows|narrowed)|merg(?:e|es|ed|ing|er|ers)|acquisitions?|acquir(?:e|es|ed|ing)|takeovers?|bankrupt(?:cy|cies)?|\bipo\b|initial public offerings?)\b/i,
...
const GDELT_CORPORATE_RECORD_LOSS_PATTERN = /(?:\b(?:companies?|corporate|business|banks?|lenders?|retailers?|manufacturers?|automakers?|airlines?|firms?|plc|inc|corp|earnings|revenue|sales|loans?|defaults?|fiscal)\b.{0,80}\brecord loss(?:es)?\b|\brecord loss(?:es)?\b.{0,80}\b(?:companies?|corporate|business|banks?|lenders?|retailers?|manufacturers?|automakers?|airlines?|firms?|plc|inc|corp|earnings|revenue|sales|loans?|defaults?|fiscal)\b)/i;
...
const GDELT_LANE_HEADLINE_PATTERNS: Record<GdeltDiscoveryLaneId, RegExp> = {
  ...
  companies_technology: /\b(?:earnings|revenue|profits?|(?:annual|quarterly|operating|net) loss(?:es)?|loss(?:es)? (?:widen|widens|widened|narrow|narrows|narrowed)|quarterly results?|merg(?:e|es|ed|ing|er|ers)|acquisitions?|acquir(?:e|es|ed|ing)|takeovers?|bankrupt(?:cy|cies)?|\bipo\b|initial public offerings?|companies|corporate|semiconductors?|cybersecurity|artificial intelligence|technology|software|cloud computing)\b/i,
...
function galRelevanceScore(title: string): number {
  ...
  return governedSignals
    + (GDELT_MARKET_MOVEMENT_PATTERN.test(title) ? 1 : 0)
    + (GDELT_CORPORATE_RECORD_LOSS_PATTERN.test(title) ? 1 : 0);
}
...
function gdeltHeadlineMatchesLane(title: string, laneId: string): boolean {
  ...
  return GDELT_LANE_HEADLINE_PATTERNS[laneId as GdeltDiscoveryLaneId].test(title)
    ...
    || (laneId === "companies_technology" && GDELT_CORPORATE_RECORD_LOSS_PATTERN.test(title));
}
