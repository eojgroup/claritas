export type Instrument = { symbol: string; name: string; country: string | null; category: "country_equity_index" | "commodity" };
...
export const MARKET_INSTRUMENTS: Instrument[] = [
  ...
  ...[["GC=F","Gold"],["SI=F","Silver"],["CL=F","WTI Crude Oil"],["BZ=F","Brent Crude Oil"],["NG=F","Natural Gas"],["HG=F","Copper"],["ZC=F","Corn"],["ZW=F","Wheat"]].map(([symbol,name]) => ({ symbol, name, country: null, category: "commodity" as const })),
];

export type ChartPoint = { date: string; observedAt: string; value: number };
export function parseYahooChart(payload: unknown): ChartPoint[] {
  ...
  return timestamps.flatMap((timestamp, index) => {
    ...
    if (!Number.isFinite(seconds) || closes[index] == null || !Number.isFinite(value)) return [];
