...
export type NewsCountryInferenceInput = {
  title?: string | null;
  summary?: string | null;
  content?: string | null;
  keywords?: string[] | null;
  url?: string | null;
  feedCountryHint?: string | null;
  localeHint?: string | null;
};

export type CountryInferenceSource =
  | "content_alias"
  | "feed_hint"
  | "locale_hint"
  | "url_tld"
...
export type CountryInferenceConfidence = "high" | "medium" | "low" | "none";

export type CountryInferenceResult = {
  iso2: string | null;
  source: CountryInferenceSource;
...
const ALIAS_MATCHERS: AliasMatcher[] = COUNTRY_ALIASES.flatMap(({ iso2, aliases }) =>
  aliases.map((alias) => ({
    iso2,
    alias,
...
function extractUrlText(url?: string | null): string {
  if (!url) return "";
  try {
...
export function inferIso2FromUrl(url?: string | null): string | null {
  if (!url) return null;
  try {
    const parsed = new URL(url);
