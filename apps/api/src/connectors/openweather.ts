...
export async function upsertWeatherSnapshot(params: {
  sourceId: number;
  countryIso2: string;
  coord?: { lat?: number | null; lon?: number | null };
  main?: OWCurrent["main"];
  wind?: OWCurrent["wind"];
  weather?: OWCurrent["weather"];
...
export async function ingestOpenWeatherCountryCurrent(countryIso2?: string) {
  const apiKey = process.env.OPENWEATHER_API_KEY || "";
  if (!apiKey) throw new Error("OPENWEATHER_API_KEY not set");

  const source = await ensureSource("openweather", "https://api.openweathermap.org");

  // Build target list of countries and approximate coords.
  // If a specific country is requested, hit only that.
  type Target = { iso2: string; lat: number; lon: number };
