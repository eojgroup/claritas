...
export type BriefingEmailTheme = "light" | "dark";

type BriefingMapPalette = {
  background: string;
  border: string;
  countryLow: [number, number, number];
  countryHigh: [number, number, number];
  countryEmpty: string;
  featuredCountry: string;
  featuredCountryBorder: string;
  ink: string;
  muted: string;
  attribution: string;
  marker: string;
...
const MAP_PALETTES: Record<BriefingEmailTheme, BriefingMapPalette> = {
  light: {
    background: "#F3E9D7",
    border: "#87979E",
...
function countryFill(
  relevance: number,
  featured: boolean,
  palette: BriefingMapPalette
): string {
  if (featured) return palette.featuredCountry;
  if (relevance <= 0) return palette.countryEmpty;
  const ratio = Math.max(0, Math.min(1, relevance / 100));
  const rgb = palette.countryLow.map((channel, index) =>
    Math.round(channel + (palette.countryHigh[index] - channel) * ratio)
  );
  return `rgb(${rgb.join(",")})`;
}

export function renderBriefingMapSvg(
  countries: BriefingMapCountry[],
  theme: BriefingEmailTheme = "dark"
): string {
  ...
  const paths = WORLD_FEATURES.map((countryFeature) => {
    ...
    return `<path d="${geometryPath(countryFeature.geometry)}" fill="${countryFill(
      ...
      isFeatured,
      palette
    )}" stroke="${isFeatured ? palette.featuredCountryBorder : palette.border}" stroke-width="${
      isFeatured ? 2.2 : 0.8
    }" fill-rule="evenodd"/>`;
  }).join("");
  ...
  const featuredText = featured
    ? `<text x="36" y="35" fill="${palette.marker}" font-family="Times New Roman, Times, serif" font-size="15" font-weight="700" letter-spacing="2.5">HIGHEST BRIEFING RELEVANCE</text>
       <text x="36" y="62" fill="${palette.ink}" font-family="Times New Roman, Times, serif" font-size="23" font-weight="700">${escapeXml(
         featured.country_name
       )} · ${escapeXml(featured.country_iso2)}</text>
       <text x="${WIDTH - 36}" y="52" text-anchor="end" fill="${palette.muted}" font-family="Times New Roman, Times, serif" font-size="16">Relevance ${Math.round(
...
export async function renderBriefingMapPng(
  ...
  return sharp(Buffer.from(renderBriefingMapSvg(countries, theme)))
