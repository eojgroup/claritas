...
type BriefingContext = {
  ...
  intelligence_events: BriefingPriorityEvent[];
};
...
const PROMPT_VERSION = "daily-signal-briefing.v10";
...
async function collectBriefingContext(options: Required<Pick<DailyBriefingGenerationOptions, "briefingDate">> &
  ...
  const [
    ...
    getBriefingIntelligenceEvents(start, end, 24),
  ]);

  const intelligenceEvents = projectBriefingPriorityEvents(intelligenceEventResult);
  ...
  const markets = [...marketOverview.countries]
    ...
    .map((row) => ({
      ...
      country_index: row.index_symbol ? {
        ...
        name: row.index_name,
        level: row.index_value,
        previous_level: row.index_previous_value,
        percent_change: row.index_change_percent,
        period_end: row.index_period_end,
      ...
      currency_vs_eur: row.fx_symbol ? {
        ...
        rate: row.fx_rate,
        previous_rate: row.fx_previous_rate,
  ...
  const weather = weatherRows.slice(0, weatherLimit).map((row) => ({
    ...
    representative_location: row.location_name,
...
function buildSystemPrompt(): string {
  return [
    ...
    "Use supplied intelligence_events as the primary cross-domain synthesis layer. For each material event explain what happened, where and exactly when it is located in the evidence timeline, why it matters, which publisher reports are linked, and what Earth-observation evidence exists. Their evidence relationship labels distinguish reported, observed, derived, interpreted, and assessed material; correlation never establishes causation.",
    "Cover News, Podcast Intelligence, Markets, Weather, Earth Observation, and Transport when material evidence is available. Treat linked_news titles, publishers, publication times and URLs as the event's auditable news linkage. If an event has no linked publisher report, say so explicitly and do not invent impact. Distinguish sensor observations, visual context and model interpretations; imagery is never automatic confirmation. A natural-colour image of fields, vegetation, water or buildings does not confirm or disprove a fire, disruption or damage. State acquisition alignment and the supplied assessment_boundary; only describe visible impact when an explicitly labelled interpretation supports it. Use the raw domain collections for provenance and detail, not to recreate a competing correlation graph. If a category has thin, stale, or missing data, say that plainly.",
...
export async function generateDailySignalBriefing(
  ...
  const payload: GeneratedDailySignalBriefingPayload = {
    ...
    metadata: {
      ...
      priority_events: context.intelligence_events,
