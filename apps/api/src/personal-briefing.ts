...
async function selectPersonalIntelligenceEvents(
  preferences: PersonalBriefingPreferences,
  sourceWindowStart: string,
  sourceWindowEnd: string,
  ...
  const terms = [...preferences.industries, ...preferences.company_symbols].map((value) => value.toLowerCase());
...
function deterministicBriefing(
  ...
  intelligenceEvents: PersonalIntelligenceEvent[],
  ...
  const updateText =
    intelligenceEvents.length > 0
      ? `${intelligenceEvents.length} correlated event${intelligenceEvents.length === 1 ? "" : "s"} matched ${tracked}. The highest-relevance event is “${intelligenceEvents[0].title}” at ${intelligenceEvents[0].where}${intelligenceEvents[0].start_time ? `, recorded from ${intelligenceEvents[0].start_time}` : ""} (${intelligenceEvents[0].severity}, ${Math.round(intelligenceEvents[0].confidence * 100)}% confidence): ${intelligenceEvents[0].what}${intelligenceEvents[0].linked_news.length ? ` ${intelligenceEvents[0].linked_news.length} linked publisher ${intelligenceEvents[0].linked_news.length === 1 ? "report is" : "reports are"} retained for provenance.` : " No publisher report is explicitly linked, so real-world impact remains uncontextualised by news."}${intelligenceEvents[0].earth_observation.length ? ` Earth-observation context is explicitly labelled by evidentiary role. ${intelligenceEvents[0].earth_observation[0].assessment_boundary ?? "Imagery is not automatic impact confirmation."}` : " No event-linked imagery is available."} Correlation supplies context and does not establish causation.${macroSentence ? ` Supporting context: ${macroSentence}.` : ""}${transportSentence}`
      : signals.length > 0
...
async function generateBriefingCopy(
  ...
  if (
    ...
    return deterministicBriefing(briefingDate, preferences, signals, markets, transport, macro, intelligenceEvents);
  }
...
async function generateForJob(job: PersonalBriefingJobRow): Promise<number> {
  ...
  const [signals, intelligenceEvents] = await Promise.all([
    selectSignals(
      preferences,
    ...
    selectPersonalIntelligenceEvents(
      preferences,
      sourceWindowStart.toISOString(),
      sourceWindowEnd.toISOString(),
    ),
...
function toEmailContent(row: DeliveryClaimRow): BriefingEmailContent {
  ...
  const events: BriefingEmailEvent[] = priorityEvents.slice(0, 8).flatMap((value) => {
    ...
    return [{
      ...
      start_time: toIso(boundedText(event.start_time, 50) || null),
      ...
      earth_observation: observations.slice(0, 4).flatMap((observationValue) => {
        ...
        return productType ? [{
          ...
          temporal_alignment: boundedText(observation.temporal_alignment, 500) || null,
          assessment_boundary: boundedText(observation.assessment_boundary, 500) || null,
        }] : [];
      }),
    }];
  });
