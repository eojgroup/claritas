...
1. Create an Apple Push Notification authentication key and store the complete Production P-256 `.p8` PEM as `APNS_PRIVATE_KEY`, with Apple's 10-character uppercase alphanumeric `APNS_KEY_ID` and `APNS_TEAM_ID`, in GitHub Actions secrets. For Apple's newer environment-scoped keys, store the Sandbox pair as `APNS_SANDBOX_PRIVATE_KEY` and `APNS_SANDBOX_KEY_ID`; supply both or neither. Older keys that explicitly support both environments may omit the Sandbox pair. Invalid formats are treated as not configured.
2. Confirm deployment creates `claritas-apns` only when the Production values and team ID are present and any Sandbox pair is complete. Omitted GitHub values deliberately preserve an existing Kubernetes secret; they do not disable or erase it. Use `APNS_DELIVERY_ENABLED=false` or an explicit rotation/revocation to stop existing credentials. The committed topic is `APNS_BUNDLE_TOPIC=com.eojgroup.claritas`; it must exactly match the iOS bundle identifier and the device registration payload. `.cer` files are public certificates and cannot replace the `.p8` private signing key (certificate authentication instead requires an exported `.p12` identity).
...
8. Open the notification and confirm the app navigates to the exact event investigation. Keep in-app acknowledgement distinct from transport acceptance.
