...
The current event list is prioritized by relevance and activity and excludes resolved, dismissed, or time-expired events by default. Every event response exposes `start_time`, `last_activity_time`, computed `expires_at`, and `freshness_state`. Reader-facing visibility is bounded from the last real activity: 24 hours for market/reported developments, 36 hours for earthquakes, 48 hours for wildfire/transport/security and unclassified events, 72 hours for flood/storm systems, and seven days for agricultural stress. `include_expired=true` is an explicit archive view; it never makes archived events current again.

Detail responses expose hydrated source titles, summaries and URLs where available; evidence grouped by domain; locations and related events; EO observations/assets; and epistemic notices. News can deep-link to its canonical event, while the event timeline links back to the original story. Event-specific imagery appears inside the investigation; on web this can include direct NASA GIBS browse context as well as persisted observations. The separate Imagery library is a provenance catalogue, not a parallel intelligence workflow.
...
Outbox, correlation decisions, evidence and job/delivery state remain auditable. Event expiry removes a record from current reader surfaces; it does not delete its evidence or audit history. The EO bucket lifecycle is 60 days and `EO_ASSET_RETENTION_DAYS` may shorten—but never extend—that lifetime. Reused immutable objects preserve their original lifecycle deadline in database expiry. Raw transport retention remains defined by the transport architecture. Event records and evidence remain in Cloud SQL until an explicit storage-retention policy supersedes this foundation.
