...
- MMSI Maritime Identification Digits link a vessel to its flag country. Position-in-country, the first observed voyage country, and recognizable AIS destination/UN LOCODE values add current, origin, and destination relationships.
...
| --- | --- | --- |
| Current | Natural Earth polygon containing the AIS position | Natural Earth polygon containing the ADS-B position |
...
- `GET /api/transport/overview?detail=aggregate` returns KPIs, country aggregates, corridor aggregates, hourly activity, freshness, and source coverage. iPhone and Watch use this response.
...
- Web and iPad show current vehicles, route curves, a selected sampled track, flight number or MMSI, country-chain drill-in, a 24-hour activity chart, most-connected-country bars, corridor flows, and a dense identifier table.
- iPhone shows only KPIs, leading countries, and leading corridors. It does not receive raw vehicles or track points during normal app bootstrap.
- Watch shows only aggregate flight/vessel/country counts, an aggregate country bubble map, and the leading corridor, with a handoff to the iPhone transport section.
